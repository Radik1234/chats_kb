...
| `BACKFILL_ERROR_BACKOFF_SEC` | `900` | Базовый backoff при ошибке (удваивается за каждый сбой подряд, потолок 64×) |
| `BACKFILL_GUARD_INBOX` | `true` | Пропускать цикл, если в `inbox/` уже есть файл |

## Недоступные чаты и ошибки

Если выбранный (самый отстающий) чат недоказуем для аккаунта — вышли/забанили,
чат удалён, entity не резолвится — `fetch` бросает исключение. Воркер это
**не** ретраит вхолостую каждый цикл: он фиксирует `error_count`, `last_error` и
`retry_after` в `state/backfill_state.json` и **паркует** чат на эскалирующий
backoff (`BACKFILL_ERROR_BACKOFF_SEC` × 2^(сбои−1), потолок 64×). Пока чат в
backoff, `select_chat` его пропускает, поэтому **остальные чаты продолжают
дозагружаться** (недоступный чат не блокирует очередь). После успешного прогона
счётчик ошибок и `retry_after` сбрасываются.
