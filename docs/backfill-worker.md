...
дозагружаться** (недоступный чат не блокирует очередь).

Backoff **временный, не постоянный**: как только `retry_after` истёк, чат снова
становится кандидатом (а так как его `date_max` за простой не двигался — он
остаётся «самым отстающим» и берётся первым). Если к этому моменту чат снова
доступен, `fetch` проходит, `error_count`/`retry_after`/`last_error`
сбрасываются, и **дозагрузка продолжается с той же границы `max(message_id)`** —
без потерь и дублей. Рост backoff ограничен потолком 64×, так что даже надолго
недоступный чат перепроверяется (дёшево) и не забывается навсегда.
